...
public:
	...
	void Insert(int data){
		...
		if(p->data_ >= data)
			p->l_son_ = new Node(data);
		else
			p->r_son_ = new Node(data);
	}
	...
	void RemoveInvterval(int lower, int upper){
		...
		for(; q;){
			...
			if(q->data_ < lower){
				...
				if(q->data_ == lower || q->data_ == upper)
					return;
				Remove(p, q, 1);
				return;
			}
...
SearchTree tree;
int total_command_number, parameter, parameter2;
char command[30];
SearchTree::Node *tmp;
int main(){
	...
	for(int i = 0; i < total_command_number; ++i){
		...
		if(!strcmp(command, "insert"))
			...
			tree.RemoveLessThan(parameter);
		else if(!strcmp(command, "delete_greater_than"))
			tree.RemoveGreaterThan(parameter);
		else if(!strcmp(command, "delete_interval")){
			scanf("%d", &parameter2);
			tree.RemoveInvterval(parameter,parameter2);
		}
		else if(!strcmp(command, "find"))
			printf("%c\n", tree.Find(parameter) ? 'Y' : 'N');
		else if(!strcmp(command, "find_ith")){
			tmp = tree.FindIth(parameter);
			if(tmp)
				printf("%d\n", tmp->data_);
			else
				printf("N\n");
